...
def _process_one_key(key, files, all_info_dict, dirpath):
    ...
    bib_info_this_key = []
    for f in files:
        if os.path.splitext(f)[1].lower() == '.ipynb':
...
def collect_bib_info(start_dir):
    ...
    for dirpath, dirs, files in os.walk(start_dir):
        ...
        not_valid_key = any(k.startswith('_') for k in key)

        if not not_valid_key:
...
def build_ref_lib(website_root,
                  source_url_root, subdir='reference_library',
                  name='reference library'):
    lib_root = os.path.join(os.path.split(__file__)[0], '..', 'src', subdir)
...
def main():
    source_url_root = 'https://nbviewer.jupyter.org/url/leelabcnbc.github.io/lab-wiki'
    website_root = 'https://leelabcnbc.github.io/lab-wiki'
    summary_dict_ref_lib = build_ref_lib(website_root + '/reference_library',
                                         source_url_root + '/reference_library')

    summary_dict_misc_notes = build_ref_lib(website_root + '/misc_notes',
                                            source_url_root + '/misc_notes', subdir='misc_notes',
                                            name='Miscellaneous Notes')

    summary_dict_ongoing = build_ref_lib(website_root + '/ongoing',
                                         source_url_root + '/ongoing', subdir='ongoing',
                                         name='Ongoing')

    # print(json.dumps(summary_dict_ref_lib, indent=2))
    summary_dict_all = OrderedDict()
    summary_dict_all['reference_library'] = summary_dict_ref_lib
    summary_dict_all['misc_notes'] = summary_dict_misc_notes
    summary_dict_all['ongoing'] = summary_dict_ongoing

    with open(os.path.join(os.path.split(__file__)[0], '..', 'src', 'summary.json'), 'w', encoding='utf-8') as f_out:
